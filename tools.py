...
def save_dict(dict):
    writer_dict = codecs.open('./code/dict.txt', "w", encoding='utf-8', errors='ignore')
    for each in dict:
        writer_dict.write(each+"\n")
    writer_dict.flush()
    writer_dict.close()

#load dict from local
#Input: @local path of the dict
#Output:@dict of relevant model
def load_dict(path):
...
def load_embedding(dict):
    ...
    with open('./code/segment/chinese-char500.bin', 'rb')as f:
        header = f.readline()
...
def define_model(MAX_SEQUENCE_LENGTH, embedding_matrix, nb_words, EMBEDDING_DIM, N_label):
    ...
    # Model 3 based on LSTM model dropout
    #sequence = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='int32')
    ...
    #blstm = Bidirectional(LSTM(100), merge_mode='concat')(embedded)
    ...
    # Model 4 based on Attention+Bi-lstm
    # sequence = Input(shape=(MAX_SEQUENCE_LENGTH,), dtype='int32')
    # embedded = Embedding(nb_words, EMBEDDING_DIM, weights=[embedding_matrix], input_length=MAX_SEQUENCE_LENGTH,
    #                      mask_zero=True, trainable=True)(sequence)  #
...
#        @saved model path
# Output:@array X for text; array y for labels
def train_model(model, X_train, y_train, X_val, y_val, sample_weights, path, weight):
    early_stopping = EarlyStopping(monitor='val_loss', patience=3)
    bst_model_path = path+'.h5'
    model_checkpoint = ModelCheckpoint(bst_model_path, save_best_only=True, save_weights_only=False)
    model.fit(X_train, y_train, sample_weight=sample_weights, batch_size=50, epochs=5, class_weight=weight, shuffle=True, validation_data=(X_val, y_val),
...
#Predict the type of the comment
# Input: @original text
#        @local dict
#        @local model
#        @ID to label transition table
